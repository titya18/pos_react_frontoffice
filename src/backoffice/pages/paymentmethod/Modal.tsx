...
interface ModalProps {
    ...
    paymentMethod?: { id: number | undefined, name: string } | null;
};
...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, onSubmit, paymentMethod }) => {
    ...
    useEffect(() => {
        if (paymentMethod) {
            ...
            reset({ name: paymentMethod.name });
        } else {
            reset({ name: '' });
        }
    }, [paymentMethod, setValue, reset]);

    const handleFormSubmit = async (data: FormData) => {
        setIsLoading(true);
        try {
            // Call the onSubmit function, making sure it recieve the correct format
            await onSubmit(paymentMethod?.id || null, data.name);
            reset();
            onClose();
        } catch (error) {
            ...
            <div className="flex items-center justify-center min-h-screen px-4">
                <div className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                    <div className="flex bg-[#fbfbfb] dark:bg-[#121c2c] items-center justify-between px-5 py-3">
                        <h5 className="font-bold text-lg">{paymentMethod ? "Edit Payment Method" : "Add New Payment Method"}</h5>
                    ...
                    <form onSubmit={handleSubmit(handleFormSubmit)}>
                        <div className="p-5">
                            <div className="dark:text-white-dark/70 text-base font-medium text-[#1f2937]">
                                <label htmlFor="module">Payment Method's Name <span className="text-danger text-md">*</span></label>
                                <input 
                                    type="text" 
                                    placeholder="Enter Payment Method's name" 
