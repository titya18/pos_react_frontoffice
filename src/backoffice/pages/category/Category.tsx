...
const Category: React.FC = () => {
    ...
    const fetchCategories = async () => {
        setIsLoading(true);
        try {
            const { data, total } = await apiClient.getAllCategories(currentPage, searchTerm, itemsPerPage, sortField, sortOrder);
            setCategories(data);
            setTotalItems(total);
    ...
    const handleEditClick = (categoryData: CategoryData) => {
        setSelectCategory({
            id: categoryData.id,
