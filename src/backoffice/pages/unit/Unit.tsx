...
import * as apiClient from "../../../api/unit";
...
const Unit: React.FC = () => {
    ...
    const [selectUnit, setSelectUnit] = useState<{ id: number | undefined, name: string} | null>(null);
    ...
    const fetchUnits = async () => {
        setIsLoading(true);
        try {
            const { data, total } = await apiClient.getAllUnits(currentPage, searchTerm, itemsPerPage, sortField, sortOrder);
            setUnits(data);
            setTotalItems(total);
    ...
    const handleAddorEditUnit = async (id: number | null, name: string) => {
        try {
            ...
            await apiClient.upsertUnit(unitData);
            if (id) {
    ...
    const handleEditClick = (unitData: UnitData) => {
        setSelectUnit({
            id: unitData.id,
            name: unitData.name
        });
                                ...
                                <div className="md:absolute md:top-0 ltr:md:left-0 rtl:md:right-0">
                                    <div className="mb-5 flex items-center gap-2">
                                        {hasPermission('Unit-Create') &&
                                            <button className="btn btn-primary gap-2" onClick={() => { setIsModalOpen(true); setSelectUnit(null) }}>
                                                <svg
